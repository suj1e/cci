...
export class FeishuBridge {
  ...
  private handleFeishuMessage(data: any): void {
    ...
    if (this.localServer.hasCliConnection()) {
      ...
      this.logger.info(
        `[BRIDGE] Sending user_message to CLI: "${message.content.substring(0, 50)}..."`,
      );
      this.logger.info(
        `[BRIDGE] Full message: ${JSON.stringify(bridgeMessage)}`,
      );
  ...
  private handleCliResponse(content: string): void {
    ...
    this.feishuClient
      .sendMessage(this.currentUserId, feishuPost)
      .catch((error) => {
  ...
  private flushPendingBuffer(): void {
    ...
    this.logger.info(
      `[STREAM] Sending accumulated content, length=${content.length}`,
    );
    ...
    if (OutputFormatter.shouldUseCard(content)) {
      ...
      cards
        ...
        .catch((error) => {
          ...
          this.feishuClient
            .sendRichMessage(userId, richContent)
            .catch((err) => {
      ...
      // 使用 Post 格式发送
      const richContent = OutputFormatter.toFeishuPost(content);
      this.feishuClient.sendRichMessage(userId, richContent).catch((error) => {
        this.logger.error("Failed to send rich message to Feishu:", error);
        // 降级为纯文本发送
        this.feishuClient.sendMessage(userId, content).catch((err) => {
  ...
  private handleStreamEnd(): void {
    ...
    if (this.streamBuffer.length > 0 && this.currentUserId) {
      const userId = this.currentUserId;
      const remainingContent = MessageConverter.mergeStreamChunks(
        this.streamBuffer,
      );
      ...
      if (cleanContent.trim()) {
        ...
        if (OutputFormatter.shouldUseCard(cleanContent)) {
          ...
          this.feishuClient
            ...
            .catch((error) => {
              this.logger.error(
                "Failed to send final message to Feishu:",
                error,
              );
            });
        }
